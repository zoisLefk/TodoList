<div class="container">
    <button command="show-modal" commandfor="edit-todo-{{ $todo->id }}" class="shadow-inner shadow-black p-4 rounded-lg bg-white/10 hover:bg-white/5">
            ...
            <div tabindex="0" class="flex min-h-full items-end justify-center p-4 text-center focus:outline-none sm:items-center sm:p-0">
                <el-dialog-panel class="relative transform overflow-hidden rounded-lg bg-gray-800 text-left shadow-xl outline -outline-offset-1 outline-white/10 transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-lg data-closed:sm:translate-y-0 data-closed:sm:scale-95">
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <h3 id="dialog-title" class="text-base font-semibold text-white">Edit Task</h3>
                            <input type="hidden" name="project_id" value="{{ $project->id }}">

                            <div class="mb-3">
                                <label for="title" class="form-label text-white">Title</label>
                                <input id="title" name="title" value="{{ $todo->title }}" style="color: black" class="form-control text-black" required>
                            </div>

                            <div class="mb-3">
                                <label for="description" class="form-label text-white">Description</label>
                                <textarea id="description" name="description" class="form-control" style="color: black;">{{ $todo->description }}</textarea>
                            </div>
                    </div>
                    </div>
